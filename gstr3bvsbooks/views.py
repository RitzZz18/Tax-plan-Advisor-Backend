...
def normalize_helper_data(helper_df):
    ...
    for _, r in helper_df.iterrows():
        if r.get("Export_Taxable", 0) > 0:
            sup_cat = "EXPWP" if r.get("IGST", 0) > 0 else "EXPWOP"
            taxable = r["Export_Taxable"]
        elif r.get("SEZ_Taxable", 0) > 0:
            sup_cat = "SEZWP" if r.get("IGST", 0) > 0 else "SEZWOP"
            taxable = r["SEZ_Taxable"]
        elif r.get("Nil_Rated", 0) > 0 or r.get("Exempt", 0) > 0:
            sup_cat = "NIL"
            taxable = r.get("Nil_Rated", 0) + r.get("Exempt", 0)
        elif r.get("Non_GST", 0) > 0:
            sup_cat = "NON_GST"
            taxable = r["Non_GST"]
        else:
            sup_cat = "DOM"
            taxable = r.get("Domestic_Taxable", 0)
        
        normalized_rows.append({
            "SUP_CAT": sup_cat, "Taxable": taxable,
            "IGST": r.get("IGST", 0), "CGST": r.get("CGST", 0), 
            "SGST": r.get("SGST", 0), "Is_RCM": r.get("Is_RCM", "N")
        })
